...
window.videoPlayer = (() => {    // --- PRIVATE VARIABLES (Scoped to this module) ---
    ...
    // Initial setup of all event listeners, runs once.
    function bindEvents() {
    ...
    function toggleFullscreen() {
        ...
        else { playerContainer.requestFullscreen().catch(err => console.error(`Fullscreen error: ${err.message}`)); }
    }
    function toggleMute() {
        if (video.volume > 0) { lastVolume = video.volume; video.volume = 0; }
        else { video.volume = lastVolume; }
    }
    ...
    function updateVolumeUI() {
        ...
        high.style.display = 'none'; low.style.display = 'none'; muted.style.display = 'none';
        if (video.volume === 0 || video.muted) muted.style.display = 'block';
        else if (video.volume < 0.5) low.style.display = 'block';
    ...
    function setupTrackMenus() {
        ...
        if (video.textTracks && video.textTracks.length > 0) {
            ...
            offBtn.textContent = 'Off'; offBtn.classList.add('active');
            offBtn.onclick = (e) => {
                e.stopPropagation();
                for (let j = 0; j < video.textTracks.length; j++) video.textTracks[j].mode = 'hidden';
                captionsMenu.querySelectorAll('button').forEach(b => b.classList.remove('active'));
                offBtn.classList.add('active');
                toggleMenu(captionsMenu);
            };
            captionsMenu.appendChild(offBtn);
            for (let i = 0; i < video.textTracks.length; i++) {
                const track = video.textTracks[i]; track.mode = 'hidden';
                const btn = document.createElement('button');
                btn.textContent = track.label || track.language || `Track ${i + 1}`;
                btn.onclick = (e) => {
                    e.stopPropagation();
        ...
        } else { captionsBtn.style.display = 'none'; }
    }
    function toggleMenu(menu) {
        const isVisible = menu.style.display === 'block';
        audioMenu.style.display = 'none'; captionsMenu.style.display = 'none';
    ...
    function open(videoBlob, fileName) {
        ...
        video.addEventListener('loadedmetadata', setupPlayerOnLoad, { once: true });

        playerContainer.style.display = 'block';
        document.addEventListener('keydown', handleKeyboardShortcuts);
        document.addEventListener('click', () => { audioMenu.style.display = 'none'; captionsMenu.style.display = 'none'; });
    }
