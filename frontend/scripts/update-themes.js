...
function replaceBlock(content, blockName, newContent) {
    ...
    if (!regex.test(content)) {
        console.warn(`⚠️ Marker ${startTag} not found in CSS.`);
        return content;
    }
...
function main() {
    ...
    Object.keys(themeConfig).forEach(key => {
        if (cssFiles.includes(`${key}.css`)) {
            ...
            console.warn(`⚠️ Warning: Config has theme "${key}" but "${key}.css" not found. Skipping.`);
        }
    });
...
`;
    ...
    console.log(`✅ Generated ${generatedConfigFile}`);
